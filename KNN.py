...
y = df1['Mode']
learn_data, test_data, learn_label, test_label = train_test_split(X, y, test_size=0.05, random_state=10)
...
lr__std_tn, lr_std_fp, lr_std_fn, lr_std_tp = confusion_matrix(test_label, result_label_lr_std).ravel()

print(lr_tn, lr_fp, lr_fn, lr_tp)


# 適合率（precision）・再現率（recall）の算出
print('lr_precision : %.4f'%(lr_tp / (lr_tp + lr_fp)))
print('lr_recall : %.4f'%(lr_tp / (lr_tp + lr_fn)))
print('lr_mm_precision : %.4f'%(lr_mm_tp / (lr_mm_tp + lr_mm_fp)))
print('lr_mm_recall : %.4f'%(lr_mm_tp / (lr_mm_tp + lr_mm_fn)))
print('lr_std_precision : %.4f'%(lr_std_tp / (lr_std_tp + lr_std_fp)))
print('lr_std_recall : %.4f'%(lr_std_tp / (lr_std_tp + lr_std_fn)))